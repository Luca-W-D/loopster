...
export class MusicManager extends React.Component {
    ...
    async getNextLibrary() {
        ...
        const library = await axios.get(this.nextLibraryUrl, { headers: { Authorization: `Bearer ${await tokenManager.getSafeToken()}` } }).catch(e => { console.log("failed to get library", e) })
        const libraryData = library.data
        this.nextLibraryUrl = libraryData.next
        this.library = [...this.library, ...libraryData.items.map(item => item.track)]
        this.library = this.library.filter((song, i) => this.library.map(s => s.id).indexOf(song.id) == i);
    }
    ...
    getCurrentSong() {
        console.log("cs", this.currentSong)
